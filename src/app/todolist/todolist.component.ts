...
export class TodolistComponent implements OnInit {
   ...
   */
	addTask(time, task){
    time = this.datePipe.transform(time, 'yyyy-M-d HH:mm');
    ...
    var noTask = false;
      ...
      //check for  duplicate time
			if(this.datePipe.transform(this.data.todo[i].time, 'yyyy-M-d HH:mm') === this.datePipe.transform(time, 'yyyy-M-d HH:mm')){
        this.noticeService.add({
					content: `You already have a task scheduled at ${this.datePipe.transform(this.data.todo[i].time, 'HH:mm')}`,
					icon: 'warning',
					autoClose:true, 
					close:true,
					autoCloseDelay:3000,
      ...
      //check for duplicate title
		...
		// Check is set time and task;
		if(!time){
      this.noticeService.add({
        content: `Please set the time" ${task} `,
        icon: 'warning',
        autoClose:true, 
        close:true,
        autoCloseDelay:3000,
    ...
    //add new task
    if(!duplicateTime && !duplicateTask && !noTask && !noTime){
      this.data.todo.push({id:this.data.todo.length+1,time:time, title:task,completed:false});
      this.newTask = '';
